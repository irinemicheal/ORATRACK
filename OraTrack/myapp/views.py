...
def edit_doctor(request, doctor_id):
    ...
    if request.method == "POST":
        ...
        department_id = request.POST.get('department')
        if department_id:
            # Get the DoctorDepartment instance, raise 404 if not found
            doctor.department = get_object_or_404(DoctorDepartment, pk=department_id)
        else:
...
def patient_prescriptions(request):
    ...
    appointments = Appointment.objects.filter(patient=patient, status='Approved').select_related('prescription').order_by('-appointment_date')
...
def patient_prescription_detail(request, appointment_id):
    ...
    appointment = get_object_or_404(Appointment, appointment_id=appointment_id, patient=patient, status='Approved')
...
def doctor_lab_tests(request):
    ...
    # Fetch all lab tests forwarded by this doctor (or for all patients if doctor has access)
...
def add_chemotherapy(request, id):
    ...
    chemo_list = Chemotherapy.objects.filter(appointment=appointment).order_by('-chemo_date')
    
    return render(request, 'doctor/add_chemotherapy.html', {
        'appointment': appointment,
        'chemo_list': chemo_list
    })
